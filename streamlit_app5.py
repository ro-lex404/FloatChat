# CORRECTED streamlit_app4.py - FIXED QUEUE SCOPE ISSUE
...
# CRITICAL FIX: Store the queue in session state so it persists across reruns
if 'speech_results_queue' not in st.session_state:
    st.session_state.speech_results_queue = thread_queue.Queue()
    ...
    try:
        ...
        for text_fragment in continuous_listening(stop_event, stop_phrase):
            if text_fragment and text_fragment.strip():
                print(f"📤 Putting text in queue: '{text_fragment}'")
...
def process_speech_results():
    ...
    print(f"🚀 process_speech_results called, queue size: {queue.qsize()}, queue id: {id(queue)}")
...
def start_continuous_listening():
    ...
    if st.session_state.continuous_listener is None or not st.session_state.continuous_listener.is_alive():
        ...
        # CRITICAL: Use the queue from session state
        queue = st.session_state.speech_results_queue
        ...
        st.session_state.last_speech_result = None
        st.session_state.speech_processed = False
        print(f"🎤 Started continuous listening with queue id: {id(queue)}")
...
def stop_continuous_listening():
    ...
    try:
        ...
        st.session_state.continuous_listener = None
        st.session_state.stop_listening_event = None
        print("⏹️ Stopped continuous listening")
...
# Process speech results at the start of each run
...
with tab1:
    ...
    with col1:
        ...
        if st.button("🔄 Refresh Map Data", key="refresh_map", width='stretch'):
            st.cache_data.clear()
            st.session_state.map_data = None
            st.rerun()
        ...
        # ... rest of your map dashboard code ...
        if map_data:
...
with tab2:
    ...
    # Process speech results BEFORE rendering the UI
    tab_speech_processed = process_speech_results()
    if tab_speech_processed:
        st.session_state.speech_processed = True
        st.rerun()
    ...
    with col_button:
        ...
        if st.session_state.get("listening", False):
            if st.button("Stop Recording", key="stop_continuous", width='stretch'):
                stop_continuous_listening()
                process_speech_results()
                st.rerun()
        else:
            if st.button("🎤 Record", key="start_continuous", width='stretch'):
    ...
    with st.expander("🔧 Debug Info"):
        ...
        st.json({
            ...
            "speech_queue_size": queue.qsize(),
            "speech_queue_id": id(queue),
...
if st.session_state.get("listening", False):
    if st.session_state.continuous_listener and not st.session_state.continuous_listener.is_alive():
        print("⚠️ Listener thread died unexpectedly, cleaning up")
        stop_continuous_listening()
        st.rerun()