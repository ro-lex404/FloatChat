# CORRECTED streamlit_app3.py - FIXED PROCESSING ISSUE
...
from spr_speech import continuous_listening
import threading
...
if 'user_input_box_voice' not in st.session_state:
    st.session_state.user_input_box_voice = ""
...
if 'speech_processed' not in st.session_state:
    st.session_state.speech_processed = False

# Global queue for speech results
...
def process_speech_results():
    """Processes results from the speech queue with proper text accumulation."""
    ...
    while not speech_results_queue.empty():
        try:
            ...
            if result["type"] == "continuous":
                ...
                if new_fragment:
                    # KEY FIX: Read the LATEST text from state INSIDE the loop
                    current_text = st.session_state.get("user_input_box_voice", "")
                ...
                print("Processing 'stopped' message. Finalizing text.")
                
                final_text = st.session_state.get("user_input_box_voice", "")
                ...
                # Final text cleanup - remove stop phrases
                final_text = st.session_state.get("user_input_box_voice", "")
                stop_phrases_to_clean = ["stop listening", "stop listen", "stop"]
                for phrase in stop_phrases_to_clean:
                    if final_text.endswith(phrase):
                        final_text = final_text[:-len(phrase)].strip()
                        print(f"Removed stop phrase, final text: '{final_text}'")
                ...
                st.session_state.continuous_listener = None
                st.session_state.stop_listening_event = None
                print("Stopped listening state cleaned up")
                processed_something = True
                st.session_state.speech_processed = True  # Flag for rerun
...
def start_continuous_listening():
    ...
    if st.session_state.continuous_listener is None or not st.session_state.continuous_listener.is_alive():
        ...
        st.session_state.continuous_listener = threading.Thread(
            ...
            args=(speech_results_queue, st.session_state.stop_listening_event, "stop listening"),
            daemon=True
        )
...
def send_message_callback():
    ...
    user_input = st.session_state.get("user_input_box_voice", "").strip()
    if user_input:
        ...
        # Clear only the recognized text after sending
        st.session_state.user_input_box_voice = ""

def clear_chat_callback():
    """Callback to clear the chat history and input."""
    st.session_state.chat_history = []
    st.session_state.user_input_box_voice = ""
...
# Process speech results at the start of each run - ALWAYS CALL THIS
speech_processed = process_speech_results()
if speech_processed:
    st.session_state.speech_processed = True

# CSS for styling (your existing CSS remains the same)
st.markdown("""
<style>
...
.chat-message-assistant { color: black; background-color: #f0f7ff; padding: 12px 16px; border-radius: 12px; margin-bottom: 12px; border-left: 4px solid #3d85c6; font-size: 14px; }
.info-box { color: black; background-color: #e6f2ff; padding: 12px 16px; border-radius: 8px; margin-bottom: 15px; border-left: 4px solid #0b5394; font-size: 14px; }
.warning-box { color: black; background-color: #fff3cd; padding: 12px 16px; border-radius: 8px; margin-bottom: 15px; border-left: 4px solid #ffc107; font-size: 14px; }
</style>
...
# --- Map Dashboard Tab (your existing code remains unchanged) ---
with tab1:
    ...
    with col1:
        ...
        # ... rest of your map dashboard code remains exactly the same ...
        if map_data:
...
with tab2:
    ...
    with col_input:
        # Use st.session_state.user_input_box_voice as the single source of truth.
        # The 'value' parameter makes this a "controlled component".
        ...
        # This check ensures that if the user manually types,
        # it updates the same state variable.
        if user_input != st.session_state.get("user_input_box_voice", ""):
            st.session_state.user_input_box_voice = user_input
            st.rerun() # Rerun to ensure the UI is consistent with the new state
    ...
    with col_button:
        ...
        if st.session_state.get("listening", False):
            if st.button("Stop Recording", key="stop_continuous", use_container_width=True):
                ...
                # Process any remaining results immediately
        ...
        if st.session_state.get("listening", False):
            ...
            if not (st.session_state.continuous_listener and st.session_state.continuous_listener.is_alive()):
                st.warning("Listener thread stopped unexpectedly. Resetting state.")
                stop_continuous_listening()
                st.rerun()
            else:
                st.info("🎤 Listening... Say 'stop listening' to finish.")
                
                # CRITICAL FIX: Process speech results while listening
                if process_speech_results():
                    st.rerun()
                
                # Auto-refresh every 0.5 seconds while listening
                time.sleep(0.5)
                st.rerun()
    ...
    with col_send1:
        st.button(
            ...
            disabled=(not st.session_state.get("user_input_box_voice", "").strip())
        )
    ...
    with st.expander("🔧 Debug Info"):
        ...
        st.json({
            ...
            "user_input_box_voice": st.session_state.get("user_input_box_voice", ""),
            ...
            "speech_queue_size": speech_results_queue.qsize(),
            "speech_processed_flag": st.session_state.get("speech_processed", False)
        })
...
# --- FINAL CRITICAL FIX: Always check if we need to rerun due to speech processing ---
if st.session_state.get("speech_processed", False):
    print("Speech was processed, triggering rerun")
    st.session_state.speech_processed = False
    st.rerun()

# Also check if we're still listening but the thread might have died
if st.session_state.get("listening", False):
    if st.session_state.continuous_listener and not st.session_state.continuous_listener.is_alive():
        print("Listener thread died unexpectedly, cleaning up")
        stop_continuous_listening()
        st.rerun()